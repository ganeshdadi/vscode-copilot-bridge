...
const buildTooltip = (status: string, endpoint: string, tokenConfigured: boolean, reason?: string): vscode.MarkdownString => {
  ...
  if (tokenConfigured) {
    ...
    tooltip.appendMarkdown('Auth token: ⚪ not configured. Authentication is currently disabled and requests are accepted without `Authorization`.');
    tooltip.appendMarkdown('\n\n[Configure token (optional)](command:workbench.action.openSettings?%22bridge.token%22)');
  }
